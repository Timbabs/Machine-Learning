...
/**
 * A test using the flip flop evaluation function with varying
 * n values (number of vertices) from 25 to 500
 * @author Timothy Baba
 * @version 1.0
 */
public class FF_varied_N {
    /** The n value */
    ...
    public static void process(Integer[] args) {
        ...
        for (Integer n: args) {
            N = n;
            System.out.println("processing FF_varied_N N = " + N);

            FileWriter RHC_fitness = null;
            try {
                RHC_fitness = new FileWriter("./src/opt/test/optData/FF_varied_N/RHC.txt", true);
            } catch (Exception e) {
            ...
            try {
                SA_fitness = new FileWriter("./src/opt/test/optData/FF_varied_N/SA.txt", true);
            } catch (Exception e) {
                System.out.println("did not create file");
                System.exit(0);
            }
            ...
            try {
                GA_fitness = new FileWriter("./src/opt/test/optData/FF_varied_N/GA.txt", true);
            } catch (Exception e) {
                System.out.println("did not create file");
                System.exit(0);
            }
    ...
    private static void deleteFiles() {
        ...
        for(String algo: algorithms) {
            ...
            File file =  new File("./src/opt/test/optData/FF_varied_N/" + fileName);
            if (file.exists()) {
                if(file.delete())
                {
